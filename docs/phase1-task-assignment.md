...
| 领域 | 当前状态 | 差距 | 确认人 |
|---|---|---|---|
| 代码仓库 | ✅ GitHub: zi-yuan-qin/newbear | — | A |
| 数据库 | ✅ SQLite，仅 world_state + users 表 | 缺 user_profiles / session_records 表 | A |
...
| 文档 | ✅ 产品说明书 + 公司背景参数化设计文档 + Phase1设计文档 | — | A |
...
| 模块 | 目的 | 独立性 | 负责人 |
|---|---|---|---|
| 数据库扩展 | user_profiles + session_records 表，建表 + CRUD 封装 | 独立，前置依赖 | A |
...
| 跨session用户画像存储 | 存储每局人格数据，影响下局种子 | 依赖 DB 扩展 | A |
| 人格画像分析引擎 | 多session人格聚合、趋势计算、报告增强 | 独立模块 | D |
| 新API路由 | sessions 历史、replay、profile、trend | 依赖种子引擎 + 画像引擎 | A |
...
| 保留 | `backend/src/core/config/character_profiles.py` | 基础角色设定保留，仅world_factory应用modifier |
| 保留 | `backend/src/core/config/job_profiles.py` | 岗位设定保留 |
| 保留 | `frontend/web/assets/*` | 美术资产不变 |
| 重写 | `backend/src/core/config/company_profile.py` | 固定值 → 参数化模板函数 |
| 重写 | `backend/src/core/config/incidents.py` | 固定列表 → 调用 content_pool |
| 重写 | `backend/src/core/config/meeting_events.py` | 固定列表 → 调用 content_pool |
| 重写 | `backend/src/core/config/pantry_events.py` | 固定列表 → 调用 content_pool |
| 重写 | `backend/src/core/config/report_events.py` | 固定列表 → 调用 content_pool |
| 重写 | `backend/src/core/world/seed_loader.py` | 读固定文件 → 调用 seed_generator |
| 重写 | `backend/src/core/world/world_factory.py` | 固定参数 → 接受参数化种子 |
| 重写 | `backend/src/core/db/database.py` | 新增建表 |
| 重写 | `frontend/web/main.js` | 新增历史/复玩逻辑 |
| 重写 | `frontend/web/index.html` | 新增 UI 元素 |
| 重写 | `frontend/web/styles.css` | 新增样式 |
| 新增 | `backend/src/core/config/seed_generator.py` | 种子生成引擎 |
| 新增 | `backend/src/core/config/content_pool.py` | 内容池管理器 |
| 新增 | `backend/src/core/config/content_pool/__init__.py` | — |
| 新增 | `backend/src/core/config/content_pool/incidents/*.json` | 事件内容池数据 |
| 新增 | `backend/src/core/config/content_pool/meetings/*.json` | 会议内容池数据 |
| 新增 | `backend/src/core/config/content_pool/pantry/*.json` | 茶水间内容池数据 |
| 新增 | `backend/src/core/config/content_pool/reports/*.json` | 报告内容池数据 |
| 新增 | `backend/src/core/db/user_profile.py` | 用户画像 DB 操作 |
| 新增 | `backend/src/core/db/session_store.py` | 会话记录 DB 操作 |
| 新增 | `backend/src/core/world/personality_analyzer.py` | 人格分析引擎 |
| 新增 | `backend/test_seed_generator.py` | 种子生成器测试 |
| 新增 | `backend/test_content_pool.py` | 内容池测试 |
| 新增 | `backend/test_personality_analyzer.py` | 人格分析测试 |
...
| 成员 | 角色 | 技术栈 | 主要职责 | 最终产出 |
|---|---|---|---|---|
| A（队长）| 后端 + 架构 | Python | DB扩展、Shared层(company_profile/seed_loader/world_factory)、API路由集成、联调验收 | 数据库就绪、API全链路打通、3局差异验证通过 |
| B | 前端 | HTML/CSS/JS | 历史页、复玩流程、UI适配 | 用户可见的历史记录页和复玩入口 |
| C | 后端 | Python | 种子生成器、内容池及数据、config重写(incidents/meetings/pantry/report)、测试 | 种子引擎+内容池+单元测试 |
...
│     │
│     ├──▶ P0-002 [A] Shared层接口定义（seed dataclass, content_pool 协议）
│     │         │
│     │         ├──▶ BE-001 [C] 种子生成器
│     │         ├──▶ BE-002 [C] 内容池管理器
│     │         ├──▶ BE-003 [C] 内容池数据编写
...
│
├─ 后端模块齐备 → BE-009 [A] server.py 新增6个API路由
│
Week 3 ────────────────────────────────────────────────────────────
│
├─ BE-009 [A] API就绪 → FE-001 [B] 前端历史页
├─ BE-009 [A] API就绪 → FE-002 [B] 前端复玩流程
├─ FE-001 + FE-002 → FE-003 [B] 报告页底部复玩入口
│
└─ 全部模块就绪 → INT-001 [A] 全链路联调
                         │
                         └──▶ QA-001 [A] 3局差异验收
```
...
| 阶段 | 周次 | 负责人 | 任务 |
|---|---|---|---|
| Phase 1a: 队长前置 | Week 1 前半 | A | P0-001, P0-002 |
| Phase 1b: 后端并行 | Week 1 后半 ~ Week 2 | A, C, D | BE-001 ~ BE-009 |
| Phase 1c: 前端 | Week 2 后半 ~ Week 3 前半 | B | FE-001, FE-002, FE-003 |
| Phase 1d: 联调验收 | Week 3 后半 | A | INT-001, QA-001 |
...
- **移交备注**: C 和 D 可以通过 import 直接使用这些函数
...
- **修改文件**:
  ```
  backend/src/core/config/company_profile.py   (固定值 → 参数化模板函数)
  backend/src/core/world/seed_loader.py         (固定文件 → 调用生成器接口)
  backend/src/core/world/world_factory.py       (固定参数 → 接受 SessionSeed)
  ```
- **删除文件**: 无
- **API/数据契约**:

  定义 `SessionSeed` dataclass（在 `seed_loader.py` 中暴露，C 和 D 依赖此结构）:
  ```python
  @dataclass
  class SessionSeed:
      seed_id: str
...
- **验收标准**:
  - [ ] `SessionSeed` dataclass 定义完成，C/D 可 import
  - [ ] `build_company_profile()` 接受不同 params dict 返回不同公司描述
  - [ ] `load_world_seed()` 不再读固定文件，改为调用 seed_generator 接口
  - [ ] `create_initial_world_state()` 接受 seed 参数，seed 为 None 时使用默认值（向后兼容）
  - [ ] 现有 `python backend/server.py` 仍可启动（用默认种子）
- **必需测试**:
  - [ ] 测试场景1：传入不同 cash 参数，build_company_profile 返回不同描述文案
  - [ ] 测试场景2：seed=None 时 create_initial_world_state 行为与当前版本一致
  - [ ] 测试场景3：传入 seed 后创建的角色初始 stress/energy 反映 modifier 值
- **前后端集成检查**: 纯后端，无前端依赖
- **队长验收**: 自验
- **交付物**: 重写后的 `company_profile.py`, `seed_loader.py`, `world_factory.py`
- **移交备注**: 此任务产出是 C 和 D 的接口契约——C 的 seed_generator 返回 SessionSeed，D 的 personality_analyzer 消费 user_profile
...
- **验收标准**:
  ...
  - [ ] 公司描述文案中关键数字（现金、天数、团队状态词）随参数变化
  - [ ] 保留 fallback：无参数时返回默认值
- **必需测试**:
...
- **目标**: seed_loader 从读固定文件改为调用 seed_generator 接口，world_factory 接受 SessionSeed
- **负责人**: A
- **状态**: 🔴 Not Started
- **前置依赖**: BE-005, BE-001（C的种子生成器接口就绪）
- **时间**: Week 2 前半
- **影响模块**: world
- **允许编辑路径**:
  ```
...
- **验收标准**:
  ...
  - [ ] 向后兼容：不传 seed 时使用默认种子
- **必需测试**:
  - [ ] 测试场景1：传入 seed → WorldRuntimeState 各字段反映 seed 参数
  - [ ] 测试场景2：两次调用 seed_generator 产生不同 seed（验证随机性）
...
| GET | `/api/profile` | 调用 user_profile.get_user_profile，返回人格画像 |
| GET | `/api/profile/trend` | 调用 personality_analyzer.get_trend，返回多session趋势数据 |
...
- **队长验收**: 需要 A 验收（检查 UI 与设计稿一致、数据渲染正确）
...
- **只读参考路径**:
  ```
  backend/src/core/config/company_profile.py  (build_company_profile 接口)
  backend/src/core/db/user_profile.py         (get_user_profile 接口)
  backend/src/core/world/seed_loader.py       (SessionSeed dataclass)
  docs/phase1-design.md                       (种子参数结构设计)
  ```
- **新增文件**:
  ```
...
- **移交备注**: 完成后通知 A 可以开始 BE-006（seed_loader 集成）
...
- **验收标准**:
  ...
  - [ ] pantry 至少 6 个变体条目
  - [ ] reports 至少 4 个变体模板
  - [ ] 每个条目填写 tags 和 param_conditions（C 和 A 一起 review 触发逻辑）
...
- **前后端集成检查**: 纯数据，无前端依赖
- **队长验收**: 需要 A 验收（check 内容质量和触发逻辑合理性）
...
- **负责人**: D
- **状态**: 🔴 Not Started
- **前置依赖**: P0-001（user_profile CRUD 就绪）
- **时间**: Week 1 后半 ~ Week 2
- **影响模块**: world
- **允许编辑路径**:
  ```
...
- **只读参考路径**:
  ```
  backend/src/core/db/user_profile.py         (get/update user_profile)
  backend/src/core/db/session_store.py        (list_user_sessions)
...
- **前后端集成检查**: 纯后端，A 在 BE-009 的 profile API 中消费
- **队长验收**: 需要 A 验收
- **交付物**: `personality_analyzer.py` + `test_personality_analyzer.py`
- **移交备注**: 完成后通知 A 可以集成 profile/trend API

---

#### BE-008: profile / trend API 数据层

- **目标**: 封装 profile 和 trend 的业务逻辑，供 A 在 server.py 中直接调用
- **负责人**: D
- **状态**: 🔴 Not Started
...
|---|---|---|---|---|
| Auth | POST | `/api/auth/register` | A | 修改（新增 user_profile 初始化）|
| Auth | POST | `/api/auth/login` | A | 保留 |
| Auth | POST | `/api/auth/logout` | A | 保留 |
| Auth | GET | `/api/auth/me` | A | 保留 |
| State | GET | `/api/state` | A | 修改（响应新增 seed_id）|
| World | POST | `/api/step` | A | 保留 |
| World | POST | `/api/reset` | A | 修改（使用新种子）|
| Meeting | POST | `/api/meeting/*` | A | 保留 |
| Pantry | POST | `/api/pantry/*` | A | 保留 |
| Report | POST | `/api/report/close` | A | 修改（写入 user_profile + session_record）|
| Session | GET | `/api/sessions` | A | **新增** |
| Session | GET | `/api/sessions/{id}` | A | **新增** |
| Session | POST | `/api/sessions/replay` | A | **新增** |
| Profile | GET | `/api/profile` | A（调用D的模块）| **新增** |
| Profile | GET | `/api/profile/trend` | A（调用D的模块）| **新增** |
| Debug | POST | `/api/seed/preview` | A（调用C的模块）| **新增** |
...
| 单元测试-DB层 | A | 5 | 集成在 P0-001 中 | 全部通过 |
| 集成测试 | A | 4 | `python backend/test_seed_loader.py` (重写) | 全部通过 |
| 前端验收 | B | 见 FE-001/002/003 | 浏览器手动验证 | 无 JS 报错、UI 正确 |
| 最终验收 | A | 7 | 手动全链路 + 3局差异 | 全部通过 |
...
阻塞时负责人必须立即通知 A：阻塞原因、需要什么来解除、可以并行做什么。
...
- **不得编辑其他成员的所属路径**，需要改动共享文件时必须先通知 A
- 共享文件（`server.py`, `database.py`, `runtime_state.py`）只有 A 有编辑权限
- 合入顺序遵循依赖地图：P0 → BE/FE → INT → QA

## 16. 即时下一步行动

1. A 将 `docs/phase1-design.md` 和 `docs/phase1-task-assignment.md` 发给 B/C/D
2. A 开始 P0-001（数据库建表 + CRUD）
3. A 同步开始 P0-002（SessionSeed dataclass + 接口契约）
4. C 在 P0-002 就绪后立即开始 BE-001（种子生成器）
5. D 在 P0-001 就绪后立即开始 BE-007（人格分析引擎）
6. 全体在 Week 1 结束前开一次同步会，确认接口契约和内容池数据格式
