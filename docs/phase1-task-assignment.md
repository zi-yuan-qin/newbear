...
| 重写 | `backend/src/core/db/database.py` | 新增建表（A）|
...
| 新增 | `backend/src/core/db/user_profile.py` | 用户画像 DB 操作（A）|
| 新增 | `backend/src/core/db/session_store.py` | 会话记录 DB 操作（A）|
...
| 成员 | 角色 | 技术栈 | 主要职责 | 最终产出 |
|---|---|---|---|---|
| 方坤（队长+前端）| 前端 + 验收 | HTML/CSS/JS | 前端历史页/复玩流程、全链路联调验收 | 前端UI完成、3局差异验证通过 |
| A | 后端 | Python | DB扩展、接口契约、company_profile参数化、seed_loader/world_factory重写、API路由集成 | 数据库就绪、API全链路打通 |
...
Week 3 ────────────────────────────────────────────────────────────
│
├─ BE-009 [A] API就绪 → FE-001 [方坤] 前端历史页
├─ BE-009 [A] API就绪 → FE-002 [方坤] 前端复玩流程
...
- **必需测试**:
  ...
  - [ ] 测试场景2：`SessionSeed(...)` 可正常实例化
- **队长验收**: 需要方坤验收
- **交付物**: 修改后的 `seed_loader.py`（仅 dataclass + 接口签名）
- **移交备注**: C 依赖 SessionSeed 写 seed_generator，A 自己后续在 BE-005/006 中实现
...
- **现有路由修改**:
  - `POST /api/reset` → 改用 `load_world_seed(user.session_id)`
...
- **负责人**: D
- **状态**: 🔴 Not Started
- **前置依赖**: P0-001（A 的 user_profile CRUD 就绪）
- **时间**: Week 1 后半 ~ Week 2
- **允许编辑路径**:
  ```
...
- **API/数据契约**:
  ...
  # 返回: { "openness": [65,72,68], "labels": ["第1局","第2局","第3局"] }
  ```
...
- **目标**: 封装 profile 和 trend 响应结构，供 A 在 server.py 中调用
- **负责人**: D
- **状态**: 🔴 Not Started
- **前置依赖**: BE-007
- **时间**: Week 2 后半
...
| Profile | GET | `/api/profile` | A（调用D）| **新增** |
| Profile | GET | `/api/profile/trend` | A（调用D）| **新增** |
| Debug | POST | `/api/seed/preview` | A（调用C）| **新增** |
...
1. A 将本任务分配文档发给 C / D / 方坤
2. A 开始 P0-001（数据库建表 + CRUD）
3. A 同步开始 P0-002（SessionSeed dataclass + 接口契约）
4. C 在 P0-002 就绪后开始 BE-001（种子生成器）+ BE-003（内容池数据）
5. D 在 P0-001 就绪后开始 BE-007（人格分析引擎）
6. 方坤在 BE-009 就绪后开始 FE-001（前端历史页）
